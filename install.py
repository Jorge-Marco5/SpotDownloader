...
```
ir a la carpeta donde se instalara el proyecto y ejecute:
```
    git clone https://github.com/Jorge-Marco5/SpotDownloader.git
```
Ingresar a la carpeta raiz (SpotDownloader) y corra el instalador
```
    python3 install.py
```
9-una vez que termine la instalacion de paquetes inicia el proyecto con:
```
    python3 main.py
```
...
IMPORTANTE:
En caso de obtener un error al momento de obtener los datos de las canciones o videos
asegurate de instalar ffmpeg
...
class PackageInstaller():
    ...
    def install_system_dependences(self):
        ...
        if "com.termux" in os.environ.get("PREFIX", ""):#Termux
            print("Instalando 'ffmpeg' en Termux...")
            try:
                subprocess.check_call(['pkg', 'install', 'clang', 'make', 'python', '-y'])
            ...
            print("Instalando 'python3', 'build-essential' y 'ffmpeg' en Linux...")#Linux
            try:
                subprocess.check_call(['sudo', 'apt', 'update', '-y'])
                subprocess.check_call(['sudo', 'apt', 'install', 'python3', 'build-essential', 'ffmpeg', '-y'])
            except Exception as e:
                print(f"Error instalando dependencias del sistema, intente manualmente: {e}")
        elif system == "windows":
            print("En windows puede ser que necesites instalar ffmpeg manualmente")#Windows
    ...
    def install_python_packages(self):
        ...
        try:
            ...
            if packagesNotInstaller:
                print("\nNo se instalaron los siguientes paquetes, intente manualmente:")
    ...
    def create_music_folder(self):
        try:
            if not os.path.exists("music"):
                os.mkdir("music")
                print("Carpeta 'music' creada correctamente")
            if not os.path.exists("videos"):
                os.mkdir("videos")
                print("Carpeta 'videos' creada correctamente")
        except PermissionError:
            print("\nNo se tienen los permisos necesarios para crear las carpetas en esta ubicacion")
    ...
    def run(self):
        ...
        print("\nEjecuta el comando 'python main.py' para empezar a descargar tu musica")
...
class EnvCreator:
    ...
    def create(self, variables = None):
        ...
        if variables is None:
            ...
            variables["YT_CODEC_AUDIO"] = "mp3"
            variables["YT_QUALITY_AUDIO"] = "192"
            variables["YT_QUALITY_VIDEO"] = "best"
